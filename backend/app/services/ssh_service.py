...
class SSHService:
    ...
    async def install_key(
        ...
        """Append the app public key over a password session and prove key-based login.

        Assumes ``password_from_client`` is a WORKING password: any provider-forced
        expired-password change is resolved upstream by the route (through the
        re-registration access engine) before this is called, so this path only ever
        opens a clean password session, installs the key, optionally disables password
        auth, and verifies key login.
        """
